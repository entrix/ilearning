function [net,tr,out3,out4,out5,out6]=train_p(net,varargin)
%TRAIN_P Train a neural network.
%
...
%  $Revision: 1.11.4.22.2.1 $ $Date: 2013/01/02 17:43:13 $
...
if ~isempty(varargin) && isstruct(varargin{end}) && isfield(varargin{end},'name')
  ...
  if (net.efficiency.memoryReduction ~= 1)
    varargin = [varargin {'reduction' net.efficiency.memoryReduction}];
  end
  n = numel(varargin);
  i = n + 1;
...
if ~feval(net.trainFcn,'supportsCalcModes');
  ...
  if isParallel
    ...
    warning(ws);
    WB = calcMode.getwb(calcNet,calcHints);
    net.userdata.P = calcNet.userdata.P;
  end
